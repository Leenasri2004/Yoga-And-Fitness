...
// Home dropdown click toggle
navLinks?.addEventListener('click', (event) => {
  const toggle = event.target.closest('.dropdown > a');
  if (!toggle) return;
...
// Close nav on normal link click (mobile)
...
// Instructor spotlight tabs
document.querySelectorAll('.stab').forEach(btn => {
  btn.addEventListener('click', () => {
    document.querySelectorAll('.stab').forEach(b => b.classList.remove('active'));
    document.querySelectorAll('.spotlight-panel').forEach(panel => panel.classList.remove('active'));
